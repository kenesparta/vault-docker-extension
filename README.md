...
```shell
  docker buildx build -t kenesparta/vault-docker:latest . --load
```

To install the extension, use `make install-extension` **or**:

```shell
  docker extension install kenesparta/vault-docker:latest
```
...
```shell
  docker extension dev ui-source kenesparta/vault-docker:latest http://localhost:3000
```

In order to open the Chrome Dev Tools for your extension when you click on the extension tab, run:

```shell
  docker extension dev debug kenesparta/vault-docker:latest
```

Each subsequent click on the extension tab will also open Chrome Dev Tools. To stop this behaviour, run:

```shell
  docker extension dev reset kenesparta/vault-docker:latest
```
...
```shell
docker extension update kenesparta/vault-docker:latest
```
...
```shell
docker extension rm kenesparta/vault-docker:latest
```
